...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            prepos = action.precond_pos
            preneg = action.precond_neg
            temp = set()
            for literal in prepos:
                temp.add(PgNode_s(literal, True))
            for literal in preneg:
                temp.add(PgNode_s(literal, False))

            if temp.issubset(self.s_levels[level]):
                # check for mutex
                mutex_set = [node for node in self.s_levels[level] if node in temp]
                addpos = True
                for node in mutex_set:
                    if len(node.mutex) != 0 and node.mutex.issubset(temp):
                        addpos = False
                        break
                if addpos:
                    new_node = PgNode_a(action)
                    new_node.parents = mutex_set
                    for node in mutex_set:
    ...
    def add_literal_level(self, level):
        ...
        for pgnode_a in self.a_levels[level-1]:
            ...
            temp_set = set(pgnode_a.effnodes)

            for node in temp_set:
                new_node=node
                node.parent=pgnode_a
                pgnode_a.children.add(new_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        res=False
        node1effpos=set(node_a1.action.effect_add)
        node1effneg=set(node_a1.action.effect_rem)

        node2effpos=set(node_a2.action.effect_add)
        node2effneg=set(node_a2.action.effect_rem)

        if node1effpos.issubset(node2effneg) or node2effpos.issubset(node1effneg):
            res=True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node1effnodes=node_a1.effnodes
        node1prenodes=node_a1.prenodes

        node2effnodes = node_a2.effnodes
        node2prenodes = node_a2.prenodes

        if node1effnodes.issubset(node2prenodes) or node2effnodes.issubset(node1prenodes):
            res=True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node1prenodes=node_a1.parents
        node2prenodes=node_a2.parents

        res=False

        for node in node1prenodes:
            mutex_set=node.mutex

            if len(mutex_set)!=0 & mutex_set.issubset(node2prenodes):
                res=True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        '''
        is_supp_mutex=True
        nodeact1=node_s1.parents
        nodeact2=node_s2.parents

        for node1 in nodeact1:
            for node2 in nodeact2:
                if not node1.is_mutex(node2):
                    is_supp_mutex = False

        return is_supp_mutex
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for level, slev in enumerate(self.s_levels):
